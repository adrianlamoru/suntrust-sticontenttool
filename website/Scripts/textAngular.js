...
textAngular.directive("textAngular", ['$compile', '$window', '$document', '$rootScope', '$timeout', 'taFixChrome', function($compile, $window, $document, $rootScope, $timeout, taFixChrome) {
    ...
    $rootScope.textAngularOpts = deepExtend({
        toolbar: [['h1', 'h2', 'h3', 'p', 'pre', 'quote'], ['bold', 'italics', 'underline', 'ul', 'ol', 'redo', 'undo', 'clear'], ['justifyLeft', 'justifyCenter', 'justifyRight'], ['html', 'insertImage', 'insertLink', 'unlink', 'fontColor', 'fontSize']],
		classes: {
			focussed: "focussed",
			toolbar: "btn-toolbar",
	...
	$rootScope.textAngularTools = deepExtend({
		html: {
		    display: "<button type='button' ng-click='action()' ng-class='displayActiveToolClass(active)'><i class='fa fa-code'></i></button>",
			action: function() {
		...
		sup: {
		    display: "<button type='button' ng-click='action()' ng-class='displayActiveToolClass(active)'>Sup</button>",
		    ...
		    action: function (color) {
		        ...
		        if (!editor.wrapSelection) {
		            ...
		            return editor.wrapSelection('superscript');
		        }
		    },
		    activeState: function () {
		        return $document[0].queryCommandState('superscript');
		    }
		},
		sym: {
		    display: "<button id='sym-btn-{{ this.$id }}' type='button' ng-click='action()' ng-class='displayActiveToolClass(active)'>Sym</button>",
		    tooltiptext: 'Special Symbols',
		    action: function () {
		        var editor = this.$parent;
		        var menuContainerClass = "flyout-menu-container";
		        var textEditorBtnMenuContainerClass = "text-editor-btn-menu-container";

		        if ($("." + textEditorBtnMenuContainerClass + "." + menuContainerClass).length > 0) {
		            $("." + textEditorBtnMenuContainerClass).remove();
		            return;
		        }
		        $("." + textEditorBtnMenuContainerClass).remove();

		        var menu = angular.element('<div class="' + menuContainerClass + ' ' + textEditorBtnMenuContainerClass + '"></div>');
		        ...
		        for (var i = 0; i < list.length; i++) {
		            var li = angular.element('<li id=' + list[i].value + '>' + list[i].name + '</li>');
		            li.on('click', function (event) {
		                editor.wrapSelection('insertHTML', this.id, true);
		            });
		            ul.append(li);
		        }
		...
		fontColor: {
		    display: "<button id='fontColor-btn-{{ this.$id }}' type='button' ng-click='action()' ng-class='displayActiveToolClass(active)'><i id='fontColor-letter-{{ this.$id }}' class='fa fa-font'></i><i class='fa fa-caret-down'></i></button>",
		    ...
		    action: function () {
		        ...
		        var colorLists = [
                    [{ parent: this.$id, value: '#5B0F00' }, { parent: this.$id, value: '#660000' }, { parent: this.$id, value: '#783F04' }, { parent: this.$id, value: '#7F6000' }, { parent: this.$id, value: '#274E13' }, { parent: this.$id, value: '#0C343D' }, { parent: this.$id, value: '#1C4587' }, { parent: this.$id, value: '#073763' }, { parent: this.$id, value: '#20124D' }, { parent: this.$id, value: '#4C1130' }],
                    [{ parent: this.$id, value: '#85200C' }, { parent: this.$id, value: '#990000' }, { parent: this.$id, value: '#B45F06' }, { parent: this.$id, value: '#BF9000' }, { parent: this.$id, value: '#38761D' }, { parent: this.$id, value: '#134F5C' }, { parent: this.$id, value: '#1155CC' }, { parent: this.$id, value: '#0B5394' }, { parent: this.$id, value: '#351C75' }, { parent: this.$id, value: '#741B47' }],
                    [{ parent: this.$id, value: '#A61C00' }, { parent: this.$id, value: '#CC0000' }, { parent: this.$id, value: '#E69138' }, { parent: this.$id, value: '#F1C232' }, { parent: this.$id, value: '#6AA84F' }, { parent: this.$id, value: '#45818E' }, { parent: this.$id, value: '#3C78D8' }, { parent: this.$id, value: '#3D85C6' }, { parent: this.$id, value: '#674EA7' }, { parent: this.$id, value: '#A64D79' }],
                    [{ parent: this.$id, value: '#CC4125' }, { parent: this.$id, value: '#E06666' }, { parent: this.$id, value: '#F6B26B' }, { parent: this.$id, value: '#FFD966' }, { parent: this.$id, value: '#93C47D' }, { parent: this.$id, value: '#76A5AF' }, { parent: this.$id, value: '#6D9EEB' }, { parent: this.$id, value: '#6FA8DC' }, { parent: this.$id, value: '#8E7CC3' }, { parent: this.$id, value: '#C27BA0' }],
                    [{ parent: this.$id, value: '#DD7E6B' }, { parent: this.$id, value: '#EA9999' }, { parent: this.$id, value: '#F9CB9C' }, { parent: this.$id, value: '#FFE599' }, { parent: this.$id, value: '#B6D7A8' }, { parent: this.$id, value: '#A2C4C9' }, { parent: this.$id, value: '#A4C2F4' }, { parent: this.$id, value: '#9FC5E8' }, { parent: this.$id, value: '#B4A7D6' }, { parent: this.$id, value: '#D5A6BD' }],
                    [{ parent: this.$id, value: '#E6B8AF' }, { parent: this.$id, value: '#F4CCCC' }, { parent: this.$id, value: '#FCE5CD' }, { parent: this.$id, value: '#FFF2CC' }, { parent: this.$id, value: '#D9EAD3' }, { parent: this.$id, value: '#D0E0E3' }, { parent: this.$id, value: '#C9DAF8' }, { parent: this.$id, value: '#CFE2F3' }, { parent: this.$id, value: '#D9D2E9' }, { parent: this.$id, value: '#EAD1DC' }],
                    [{ parent: this.$id, value: '#980000' }, { parent: this.$id, value: '#FF0000' }, { parent: this.$id, value: '#FF9900' }, { parent: this.$id, value: '#FFFF00' }, { parent: this.$id, value: '#00FF00' }, { parent: this.$id, value: '#00FFFF' }, { parent: this.$id, value: '#4A86E8' }, { parent: this.$id, value: '#0000FF' }, { parent: this.$id, value: '#9900FF' }, { parent: this.$id, value: '#FF00FF' }],
                    [{ parent: this.$id, value: '#000000' }, { parent: this.$id, value: '#222222' }, { parent: this.$id, value: '#444444' }, { parent: this.$id, value: '#666666' }, { parent: this.$id, value: '#888888' }, { parent: this.$id, value: '#AAAAAA' }, { parent: this.$id, value: '#CCCCCC' }, { parent: this.$id, value: '#DDDDDD' }, { parent: this.$id, value: '#EEEEEE' }, { parent: this.$id, value: '#FFFFFF' }],

		        ];

		        var btnSym = $("#fontColor-btn-" + this.$id);

		        for (var i = 0; i < colorLists.length; i++) {
		            var list = colorLists[i];
		            var ROW_HEIGHT = 18;
		            var FIRST_ROW_TOP = 25;
		            var currentRowTop = (i * ROW_HEIGHT) + FIRST_ROW_TOP;

		            var menu = angular.element('<div style= "top: ' + currentRowTop + 'px;" class="' + menuContainerClass + ' ' + textEditorBtnMenuContainerClass + '"></div>');
		            var ul = angular.element("<ul>");

		            for (var j = 0; j < list.length; j++) {
		                var li = angular.element("<li id=" + list[j].value + " style= 'background-color:" + list[j].value + "' name='" + list[j].parent + "'></li>");
		                li.on('click', function (event) {
		                    var currentItemColor = this.id;
		                    var currentItemName = $(this).attr('name');
		                    editor.wrapSelection('foreColor', currentItemColor);
		                });
		                ul.append(li);
		            }
		            menu.append(ul);
		    ...
		    activeState: function () {
		        return $document[0].queryCommandState('foreColor');
		    }
        },
        fontSize: {
            display: "<button id='font-size-btn-{{ this.$id }}' type='button' ng-click='action()' ng-class='displayActiveToolClass(active)'><i id='fontColor-letter-{{ this.$id }}' class='fa fa-text-height'></i> <i class='fa fa-caret-down'></i></button>",
            ...
            action: function () {
                ...
                var fontSizeList = [
                    { name: 'xx-small',  value: 1 },
                    { name: 'x-small', value: 2 },
                    { name: 'small', value: 3 },
                    { name: 'medium', value: 4 },
                    { name: 'large', value: 5 },
                ...
                for (var i = 0; i < fontSizeList.length; i++) {
                    var li = angular.element('<li id=' + fontSizeList[i].value + ' class="disply-block-li">' + fontSizeList[i].name + '</li>');
                    li.on('click', function (event) {
                        //editor.wrapSelection('insertHTML', this.id, true);
                        var currentItemSize = this.id;
                        return editor.wrapSelection('fontSize', currentItemSize);
                    });
                    ul.append(li);
                }
                ...
                var btnSym = $("#font-size-btn-" + this.$id);
                btnSym.append(menu);
            },
            activeState: function () {
                return $document[0].queryCommandState('fontSize');
            }
        },
		insertCommand: {
		    display: "<button id='cmd-btn-{{ this.$id }}' type='button' ng-click='action()' ng-class='displayActiveToolClass(active)'><></button>",
		    tooltiptext: 'Insert a Command',
		    action: function () {
		        var editor = this.$parent;
		        var menuContainerClass = "div-tag-container";
		        var textEditorBtnMenuContainerClass = "text-editor-btn-menu-container";
		        ...
		        var list = [
                    { name: 'Interest Offer (existing)', value: '71' },
                    { name: 'Promo Plan Number (existing)', value: '69' },
                    { name: 'Promo Duration (existing)', value: '70' },
                    { name: 'Promo APY', value: '72' },
                    { name: 'Standard Rate plan Effective Date', value: '73' },
                    { name: 'Promo Minimum Balance', value: '74' },
                    { name: 'Promo Maximum Balance', value: '75' },
                    { name: 'Standard APY', value: '76' },
                    { name: 'Standard Balance', value: '77' },
                    { name: 'Promo Effective Date', value: '78' },
                    { name: 'Promo Effective Time', value: '79' }
		        ];
        ...
        insertEmailCommand: {
            ...
            action: function () {
                ...
                var list = [
                    { name: 'Cash Back Percent', value: '81' },
                    { name: 'Cash Back Months', value: '82' },
                    { name: 'Other Cash Back Percent', value: '83' },
                    { name: 'Intro Interest', value: '84' },
                    { name: 'Intro Interest Months', value: '85' },
                    { name: 'Promo Expire Date', value: '86' },
                    { name: 'Offer Amount', value: '87' }
                ];
        ...
        insertEmailApplyCommand: {
            ...
            action: function () {
                ...
                var list = [
                    ...
                    { name: 'Promo Expire Date', value: '86' },
                    { name: 'Offer Amount', value: '87' },
                    { name: 'CTA Apply Now', value: 'CTA-APPLY-NOW' }
                ];

                for (var i = 0; i < list.length; i++) {
                    var li = angular.element('<li id=' + list[i].value + ' name=' + list[i].name + '>' + list[i].name + '</li>');
                    li.on('click', function (event) {
                       
                        var elementName = this.getAttribute('name');
                        if (elementName === 'CTA') {
                            $rootScope.$broadcast('insertApplyBtn');
                            var classe = "apply-" + $rootScope.horizontalPosition;
                            var ctaWrapper = '<span class=\"apply-now-new\">[[' + this.id + ']]</span>&nbsp;<span>&nbsp;</span>';
                            editor.wrapSelection('insertHTML', ctaWrapper, true);
                            var el = angular.element(".apply-now-new");
                            el.addClass(classe);
                            el.removeClass('apply-now-new');
                        }
                        else {
                            editor.wrapSelection('insertHTML', '[[' + this.id + ']]', true);
                        }
	...
	return {
		...
		link: function(scope, element, attrs, ngModel) {
            ...
            scope.$watch('text', function (newValue, oldValue) {
                //newValue = newValue.replace("[[CTA*Apply*Now]]", "<div class=\"apply-now\">asdfasdfasdf</div>");
				scope.html = newValue;
...
.directive('taBind', ['$sanitize', '$document', '$timeout', 'taFixChrome', function ($sanitize, $document, $timeout, taFixChrome) {
