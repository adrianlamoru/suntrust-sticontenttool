appControllers.controller('UserProfileController', function ($scope, $rootScope, $location, CommonService) {
    ...
    $scope.userId = 0;
    $scope.masterUser = { ID: -1, DateJoined: "", Email: "", Password: "", ConfirmPassword: "", FirstName: "", LastName: "", Role: -1, UserType: -1 };
    ...
    $scope.save = function () {
        if ($scope.userForm.$valid && $scope.passwordMatch()) {
            ...
            $scope.user.UserType = CURRENT_USER_TYPE_ID;
            $scope.loading = true;
    ...
    $scope.setupEditMode = function() {
        ...
        $scope.selectedUserType = $scope.getSelectedUserType($scope.user.UserType);
    };
    ...
    $scope.getSelectedUserType = function (userTypeId) {
        for (var i = 0; i < $scope.userTypes.length; i++) {
            if ($scope.userTypes[i].Value == userTypeId) {
