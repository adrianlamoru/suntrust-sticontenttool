...
textAngular.directive("textAngular", ['$compile', '$window', '$document', '$rootScope', '$timeout', 'taFixChrome', function($compile, $window, $document, $rootScope, $timeout, taFixChrome) {
	...
	$rootScope.textAngularOpts = deepExtend({
	    toolbar: [['h1', 'h2', 'h3', 'p', 'pre', 'quote'], ['bold', 'italics', 'underline', 'ul', 'ol', 'redo', 'undo', 'clear'], ['justifyLeft', 'justifyCenter', 'justifyRight'], ['html', 'insertImage', 'insertLink', 'unlink', 'fontColor']],
		classes: {
			focussed: "focussed",
			toolbar: "btn-toolbar",
		        ...
		        for (var i = 0; i < colorLists.length; i++) {
		            ...
		            for (var j = 0; j < list.length; j++) {
		                ...
		                li.on('click', function (event) {
		                    ...
		                    //$('#fontColor-letter-' + currentItemName).css({ "color": currentItemColor });
		                });
		                ul.append(li);
		            }
		            menu.append(ul);
