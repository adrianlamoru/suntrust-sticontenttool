...
appControllers.controller('ProjectLayoutController', function ($scope, $rootScope, $location, projectManager, projectService) {
    ...
    $scope.openPreviewDialogClick = function () {
        $scope.$broadcast('updateCtasWrapperClass');
    ...
    $scope.isWebProject = function () {
        return CURRENT_USER_TYPE_ID == $scope.OfferTypes.WEB_OFFER;
    };
    ...
    $scope.getSectionID = function (view) {
        if (view.indexOf('primary-banner') > -1) {
            ...
            return 11;
        } else if (view.indexOf('hero-offer') > -1) {
            return 12;
        } else if (view.indexOf('edu-offer') > -1) {
            return 13;
        } else if (view.indexOf('prod-offer') > -1) {
            return 14;
        } else if (view.indexOf('pwm-bulletin') > -1) {
            return 15;
        } else if (view.indexOf('pwm-details') > -1) {
            return 16;
        } else if (view.indexOf('pwm-primary-bann') > -1) {
            return 17;
        } else if (view.indexOf('view-all') > -1) {
            return 18;
        } else if (view.indexOf('olb-bulletin') > -1) {
            return 19;
        } else if (view.indexOf('ghost-offer') > -1) {
            return 20;
        } else if (view.indexOf('olb-learn-more') > -1) {
            return 21;
        } else if (view.indexOf('rto-primary-bann') > -1) {
            return 22;
        } else if (view.indexOf('rto-details') > -1) {
            return 23;
        } else if (view.indexOf('email-template') > -1){
            return 24;
        } else if (view.indexOf('rto-allOffers') > -1) {
            return 25;
        } 
        return 0;
    };
    ...
    $rootScope.openSelectProjectToCopyDlgClick = function () {
        $scope.$parent.areScrollsHidden = true;
        $scope.isSelectCopyProjectDlgVisible = true;

        //Posibles event's parameters (dlgTitle, dlgDescription, targetAName, targetURL)
        $scope.$broadcast('copyProjectDlgOpened', {
            dlgTitle: 'Copy content from another project',
            dlgDescription: 'Select the project to copy the content.',
            onCancelClick: $scope.closeSelectCopyProjectDlgClick,
            targetOffer: { ID: $scope.projectManager.project.ID }
        });
    };

    $scope.closeSelectCopyProjectDlgClick = function () {
