appControllers.controller('EditOffersController', function ($rootScope, $scope, $filter, CommonService, projectService, projectManager) {
    $scope.offersModel = null;
    $scope.showEmpty = false;
    $scope.OfferSelectedHasProject = false;
    ...
    $scope.setSelectedOffer = function (id) {
        ...
        //console.log("$scope.offersModel.selectedItem ", $scope.offersModel.selectedItem.ID);
        projectService.getProjectById($scope.offersModel.selectedItem.ID)
            .success(function (project) {
                $scope.OfferSelectedHasProject = true;
            })
    ...
    $scope.copyOffer = function () {
        $scope.openCopyOfferDialogClick($scope.offersModel.selectedItem);
    };
    ...
    $scope.loadData = function () {
        if ($scope.isSuperAdmin()) {
            ...
            CommonService.getOffersByOfferType(CURRENT_USER_TYPE_ID, function (data) {
                $scope.showEmpty = false;
                $scope.offersModel = new TableModel(data, $filter('orderBy'));
